...
def get_dirs():
    ...
    is a second render through the viewer's pipeline, diffed against the
...
def render_one(ps_file, output_dir, timeout, extra_flags, device="png"):
    """Render a single sample file. Copies input to output dir, runs stet,
    removes the copy, keeps the PNGs.

    `device` selects which stet render path to exercise. "png" is the
    authoritative banded-page path (baselines); "viewport-png" routes
...
def _ps_threshold(config_overrides, name, default_threshold):
    """Resolve per-sample threshold for a PS/EPS sample."""
    overrides = config_overrides or {}
    return overrides.get(f"{name}.ps",
           overrides.get(f"{name}.eps", default_threshold))
...
<strong>Comparison time:</strong> {format_duration(compare_time) if compare_time else "-"}
    ...
    for name, b_entry, vp_entry in sorted(merged, key=_row_order):
        ...
        threshold = _ps_threshold(config_overrides, name, default_threshold)
